...
| L5 | LLM synthesis | macOS + on-device Apple Intelligence (`FoundationModels`) | Key points above the report — up to three first-person sentences per repo (per-repo model calls over explicit fact blocks built from commit subjects + short body first-sentences; v0.4.0, replacing v0.3.0's single blended paragraph) |
